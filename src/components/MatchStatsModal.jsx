...
function kdColor(kills, deaths) {
  const kd = deaths > 0 ? kills/deaths : kills;
  if (kd >= 1.5) return 'var(--emerald)';
  if (kd >= 1.1) return 'var(--sky)';
  if (kd >= 0.9) return 'var(--amber)';
  return 'var(--red)';
}
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-head" style={{ background: 'var(--s2)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            <div style={{
              ...
              color:rc, border:`1px solid ${rc}`, padding:'4px 12px', borderRadius:6,
              background: `${isWin ? 'rgba(16,185,129,0.1)' : isDraw ? 'rgba(245,158,11,0.1)' : 'rgba(232,37,60,0.1)'}`,
            }}>
              {match.result?.toUpperCase()}
            </div>
            <div>
              <div style={{ fontSize:26, fontWeight:900, lineHeight:1, letterSpacing:'0.02em' }}>
                {match.score}
                <span style={{ fontSize:15, color:'var(--text3)', marginLeft:10, fontWeight:600 }}>vs {match.opponent || '—'}</span>
              </div>
              <div style={{ fontSize:11, color:'var(--text3)', fontFamily:'var(--mono)', marginTop:3, letterSpacing:'0.06em' }}>
        ...
        <div style={{ overflowY:'auto', padding:'20px 26px' }}>
          <div style={{ fontSize:9, fontFamily:'var(--mono)', color:'var(--text3)', letterSpacing:'0.15em', marginBottom:12 }}>
            PLAYER PERFORMANCE
          </div>
          ...
          {match.teamMetrics && (() => {
            ...
            return (
              <>
                <div style={{ fontSize:9, fontFamily:'var(--mono)', color:'var(--text3)', letterSpacing:'0.15em', marginTop:24, marginBottom:12 }}>TEAM METRICS</div>
                <div className="stats-modal-grid">
                  {metricCards.map(({ label, val, isText }) => (
                    <div key={label} className="stats-mini-card">
                      <div className="stats-mini-val" style={{
                        color: isText ? (val==='Win' ? 'var(--emerald)' : val==='Loss' ? 'var(--red)' : 'var(--text)') : 'var(--text)'
                      }}>{val}</div>
