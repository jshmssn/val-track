...
// ============================================================
// backend/api/ai_extract_hf.php  (Hugging Face version)
//
// UPDATED: 2026-03-04-v6-summary-scoreboard-only
// - TIMELINE is removed (no longer needed after Valorant UI update)
// - SUMMARY: extracts Round Wins (1st half / 2nd half / OT if present) + score/result
//           then computes teamMetrics in PHP (supports overtime).
// - SCOREBOARD: extracts ONLY player, agent (TEXT), ACS, K/D/A.
// ============================================================
...
// ── Config ────────────────────────────────────────────────────
$HF_API_KEY = $_ENV['HF_API_KEY'] ?? getenv('HF_API_KEY') ?: '';
...
// playerAgentMap is DEPRECATED in this new flow; we accept it but ignore it.
...
function callHuggingFace(string $apiKey, string $model, array $messages, bool $jsonMode = true): array
{
    ...
    // Some HF models ignore this, but it doesn't hurt to include for router compatibility.
    if ($jsonMode) {
...
/**
 * Compute TEAM METRICS from SUMMARY values only (supports overtime).
 * - Regulation is always 24 rounds (12 + 12)
 * - OT rounds = max(0, totalRounds - 24)
 * - atkWins/defWins split uses the ATK/DEF labels shown in SUMMARY for each half.
 */
...
- "timeline"   — Horizontal round strip (old UI; ignore if present).
...
// ──────────────────────────────────────────────────────────────
// Only SUMMARY + SCOREBOARD are supported now
// ──────────────────────────────────────────────────────────────
...
- firstHalfSide: label above 1st half box (usually "ATK" or "DEF")
- firstHalfWins: integer in the 1st half box
- secondHalfSide: label above 2nd half box (usually "ATK" or "DEF")
- secondHalfWins: integer in the 2nd half box
- overtimeWins: integer in the OVERTIME box IF present, otherwise null
...
SUMMARY);
    ...
    $rawText   = hfText(callHuggingFace($HF_API_KEY, $HF_MODEL, $summaryMessages));
    ...
    // Compute team metrics server-side
...
if (empty($extracted)) sendError('Model returned unparseable JSON for Scoreboard tab: ' . substr($rawText, 0, 300), 502);

$extracted['screenshotType'] = 'scoreboard';
$extracted['type']           = $matchType;

// Sanitize: keep only required fields and validate agent names
$cleanStats = [];
